...
export const login = async (req, res) => {
    try {
        ...
        if (existingUser.isVerified === false) {
            return res.status(400).json({
                success: false,
                message: "Verify your account then login again"
            })
        }
        ...
        // chexk for existing session and del it
...
export const verifyOTP = async (req, res) => {
    try {
        ...
        if (user.otpExpiry < new Date()) {
            return res.status(400).json({
                success: false,
                message: 'Otp has expired please request has expires please request a new one'
            })
        }
        if (otp !== user.otp) {
            return res.status(400).json({
